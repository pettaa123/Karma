...
Item {
    ...
    function pickUpDepot(){
        ...
        if(pickUp.length>5 && multiplayer.activePlayer.userId===multiplayer.localPlayer.userId){
            drawDepotSound.play()
        }
    ...
    function syncHand(messageHand) {
        var cardIDs=messageHand.handIds
        var chinaIDs=messageHand.chinaIds
        var chinaHiddenIDs=messageHand.chinaHiddenIds
    ...
    function getHandIndex(handCard){
        for (var i=0;i<hand.length;i++){
            if(hand[i].entityId===handCard) return i
        }
        return -1
    }

    function getChinaIndex(chinaCard){
        for (var i=0;i<china.length;i++){
            if(china[i].entityId===chinaCard) return i
        }
        return -1
    }
    ...
    function exchangeHandAndChinaCard(handCard,chinaCard){
        var handIndex=getHandIndex(handCard)
        //if -1 requestGameState
        var chinaIndex=getChinaIndex(chinaCard)
        //if -1 requestGameState
        var temp=hand[handIndex]
        hand.splice(handIndex,1,china[chinaIndex])
        hand[handIndex].state="player"
        ...
        hand[handIndex].hidden=true
        china[chinaIndex].hidden=false
    }
    ...
    function exchangeShakingCard(){
        ...
        if(shakingIndex!==3){
            for(i=0;i<china.length;i++){
                ...
                if(china[i].shaking){
                    ...
                    multiplayer.sendMessage(gameLogic.messageExchangeCards, {handCard: hand[shakingIndex].entityId, chinaCard: china[i].entityId,userId: userId})
        ...
        if(shakingIndex!==3){
            for(i=0;i<hand.length;i++){
                //find shaking card
                if(hand[i].shaking){
                    userId = multiplayer.localPlayer ? multiplayer.localPlayer.userId : 0
                    multiplayer.sendMessage(gameLogic.messageExchangeCards, {handCard: hand[i].entityId, chinaCard: china[shakingIndex].entityId,userId: userId})
